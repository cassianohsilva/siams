(function($) { $.fn.numberfy = function() {
	...
	$(this).filter('textarea').each(function() {
		...
		$(this).add(measurer).css({overflowY: 'auto', wordWrap: 'break-word', whiteSpace: 'pre-wrap'});

		//disable any native support for textarea resizing
		$(this).css('resize', 'none');

		//set up the line numbers col and position it to left of textarea
		var col = $('<div>').css({borderTopLeftRadius: '10px', borderBottomLeftRadius: '10px', background: column_BG, color: column_text, width: 30, position: 'absolute', overflow: 'hidden', left: 0, top: 0, height: $(this).height()}).addClass('lineNumsCol').prependTo($(this).parent());
